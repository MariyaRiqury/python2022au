...
# если потомков больше 4, то выживает лишь наиболее приспособленая к жизни (наиболее похожая на введенную пользователем) половина популяции
import random
import string
...
def select(offsprings, str):
    survival_value = map(lambda x: (match(x, str), x), offsprings)# вычисляем несхожесть потомков с идеалом
